        ...
        <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-6">
            ...
            <div class="flex items-center">
                <input id="remember_me" type="checkbox" name="remember"
                       class="w-4 h-4 rounded border-white/30 bg-white/10 text-violet-500 focus:ring-violet-500 focus:ring-offset-0">
                <label for="remember_me" class="ml-3 text-sm text-white/80">
                    {{ __('Remember me') }}
                </label>
            </div>

            <!-- Submit Button - EXACT same gradient as register -->
            <div>
                <button type="submit"
                    class="w-full justify-center py-4 text-lg font-semibold rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 transform hover:-translate-y-0.5 transition-all duration-300 shadow-xl text-white">
                    {{ __('Log in') }}
                </button>
            </div>
            ...
            <!-- Google Button - identical -->
            <div>
                <button type="button"
                    class="w-full flex items-center justify-center gap-3 py-3.5 px-4 border border-white/30 rounded-xl bg-white/5 backdrop-blur-md hover:bg-white/10 transition-all duration-300 group">
            ...
            <div class="text-center">
                <p class="text-white/70 text-sm">
                    ...
                    <a href="{{ route('register') }}" class="font-semibold text-violet-400 hover:text-violet-300 transition">
                        Sign up
                    </a>
                </p>
            </div>
